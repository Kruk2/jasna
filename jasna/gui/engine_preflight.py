...
def run_engine_preflight(settings: AppSettings) -> EnginePreflightResult:
    ...
    if bool(settings.compile_basicvsrpp) and not amd:
        ...
        if (
            ...
            and supports_fp8(device)
            and Path(get_basicvsrpp_fp8_upsample_onnx_path(str(restoration_model_path))).is_file()
        ):
            sub_paths["upsample_fp8"] = get_basicvsrpp_fp8_upsample_engine_path(
                str(restoration_model_path), int(settings.max_clip_size),
            )
