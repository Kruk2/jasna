...
JA = {
        ...
        "tip_vr_mode": "サイドバイサイド VR180 の処理方法を設定します。\n\n自動は、高さが 1080 ピクセルを超える正確な 2:1 フレーム、信頼できるスタジオ名トークン、または互換性のある空間メタデータがある場合に有効になります。SBS は左右の目を別々に検出・修復し、検出した各モザイク領域を平面パッチに変換して 2D 復元モデルで処理し、修復後のパッチを元の投影へ戻します。SBS + 魚眼も同じ処理で、既知の魚眼 VR スタジオで自動選択されます。\n\n区間エディターは左目のみ表示しますが、スキャンと書き出しは両目を処理します。\nデフォルト：自動",
