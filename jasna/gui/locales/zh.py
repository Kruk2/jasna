...
ZH = {
        ...
        "tip_vr_mode": "控制并排 VR180 视频的处理方式。\n\n自动模式会在画面为严格 2:1 且高度超过 1080 像素、文件名包含可信的片商标记或具有兼容空间元数据时启用。SBS 会分别检测和修复双眼画面，并将每个检测到的马赛克区域展平为矩形补丁交给 2D 修复模型，再将修复后的补丁投影回原始投影。SBS + 鱼眼采用相同处理，并会在已知鱼眼 VR 片商时自动选择。\n\n区间编辑器只显示左眼，但扫描和导出会处理双眼。\n默认值：自动",
