...
- サイドバイサイド VR180 動画を左右別々に処理し、検出と復元に魚眼再投影も利用できます。
...
リリースパッケージは GPU ベンダー別です。

- NVIDIA: コンピュート能力 **7.5 以上**の新しい Nvidia GPU。
...
- Nvidia ドライバー: Windows は **610.00 以上**、Linux は **580.xx 以上**。同梱の
  FFmpeg 8.1.2 ライブラリは NVENC API 13.1 を使用しており、古いドライバーではこの API を
  利用できません。古いドライバーでも CUDA 推論は初期化できる場合がありますが、
  ハードウェア動画エンコードは失敗します。
- AMD（実験的）: ROCm 対応 AMD GPU、Python 3.12、PyTorch 2.9.1。
- インストール先パスは ASCII 文字のみである必要があります。
- Windows リリースパッケージ: `ffmpeg`、`ffprobe` を同梱しています。
- Linux リリースパッケージ: `ffmpeg`、`ffprobe` を同梱しています。
...
<!-- スクリーンショットのプレースホルダー：最終版の区間エディター画像をここに追加。 -->

区間エディターでは、キュー内の動画をプレビューし、復元する範囲をフレーム単位で正確に選べます。何も選択しない場合は動画全体を復元します。**復元プレビュー**では、処理前に現在の設定で現在のフレームや短い再生結果を確認できます。
...
- スキャン後に信頼度を変更すると、琥珀色の検出区間がすぐ更新され、紫色の復元対象へ追加できます。
- サンプル済みフレームは保存マスクを表示し、未サンプルのフレームは必要時に正確に検出します。VRAM が少ない場合は、システムメモリ経由で結果を自動的に再利用します。

検出モデルと信頼度はキュー内の動画ごとに保存され、最終処理でも使われるため、動画ごとに異なる設定を選べます。

## より良いマスクを提案する

モザイク検出が正しくない場合は、修正したマスクを提供して、今後の検出モデルの改善に協力できます。区間エディタで該当フレームを一時停止し、**より良いマスクを提案**をクリックしてください：

- クリックして各モザイク領域の周りに頂点を追加します — 領域ごとに 1 つの形状で、必要な数だけ描けます。フレーム外のクリックは端に吸着します。
- 最初の頂点をクリック、ダブルクリック、または Enter で形状を閉じます。
- ホイールでズームして正確になぞり、右ドラッグでパン、H で形状を一時的に非表示、不透明度スライダーでマスクの見え方を調整できます。
- できるだけ正確に描いてください。モザイクの縁がぼやけて薄くなっている場合は、そのぼかし部分も形状に含めてください。

送信するとフレームとマスクは**匿名**でアップロードされます。データはアップロード前にローカルで暗号化され、付加されるのはアプリのバージョン、検出モデル名、フレーム解像度のみです — ファイル名やタイムスタンプなど、個人を特定できる情報は一切含まれません。

## VR180 動画

VR180 ファイルでは通常、左目用と右目用の映像が 1 枚の横長フレームに並んでいます。Jasna はフレームを分割し、左右の目を別々に復元してから、出力時に再び結合します。

### かんたん設定

1. 通常の動画と同じように VR 動画を追加します。
2. **VR180 モード**は **自動（推奨）** のままにします。
3. VR モザイクを最も正確に検出するには、同梱の **zelefans-vr-yolo-v2** 検出モデルを選びます。このモデルは Jasna に含まれているため、別途ダウンロードする必要はありません。
4. 通常どおり処理を開始します。動画の一部だけを復元したい場合は、区間エディターも使えます。

自動モードでは、高さが 1080 ピクセルを超える正確な 2:1 動画を必ずサイドバイサイド VR として扱います。たとえば、3840x1920 と 8192x4096 は自動的に認識されます。互換性のある VR メタデータや既知の VR スタジオ名から VR 処理が有効になる場合もあります。

### モードの選び方

- **自動（推奨）**: Jasna に判定を任せます。まずはこれを使ってください。
- **オフ**: 通常の平面動画として処理します。
- **SBS — 左右別処理**: 自動で認識されない場合に、サイドバイサイド VR 処理を強制します。
- **SBS + 魚眼**: VR 画像の端でモザイクが大きく引き伸ばされている場合や、通常の SBS モードで検出されない場合に使います。レンズの歪みを一時的に補正し、検出と復元を改善します。

### プレビューと出力

区間エディターでは、確認しやすい大きさで表示するため、意図的に左目だけを表示します。右目が無視されるわけではありません。モザイクスキャン、選択区間、復元プレビュー、最終出力では両目が処理されます。

VR プレーヤーとの互換性を重視する場合は、MP4 または MOV へ出力してください。Jasna は互換性のある元の VR メタデータを維持し、ない場合は標準的なサイドバイサイド VR180 メタデータを追加します。他のコンテナにも復元済みの両目が含まれますが、VR プレーヤーが自動認識できない場合があります。

**SBS + 魚眼**は内部処理用のオプションであり、完成動画を別の投影方式へ変換するものではありません。出力は元動画と同じ見た目の配置を維持します。ストリーミング動作は変わりません。

コマンドラインでも `--vr-mode` で同じモードを選べます:
...
初回実行は GPU 固有の検出アーティファクトを準備するため遅くなります。NVIDIA は TensorRT エンジンをコンパイルし（通常 **15-60 分**）、Linux AMD は RF-DETR の MIGraphX キャッシュを作成します。
...
通常は最新の RF-DETR モデルを使ってください。Lada YOLO モデルも利用でき、2D アニメーションではより良い場合があります。VR180 向けには、[ZeLeFans VR Mosaic Remover](https://huggingface.co/zelefans/vrmr) の高精度検出モデル `zelefans-vr-yolo-v2` が Jasna に同梱されています。

AMD では RF-DETR は非常に遅いため、RF-DETR が特に必要でない限り Lada YOLO（`lada-yolo-v4`）を推奨します。
...
`pyproject.toml` の Python 要件: **Python 3.12 以上**。
...
```bash
uv pip install -e ".[nvidia,dev]"  # または .[amd,dev]
```

Linux AMD リリースは専用 Docker スクリプト、Windows AMD リリースはネイティブ
ビルドスクリプトで作成します:
...
AMD では BasicVSR++/YOLO に PyTorch ROCm、RF-DETR に ONNX Runtime、
H.264/HEVC/AV1 のデコード/エンコードに AMF を使います。RF-DETR は Linux では
MIGraphX、Windows では ONNX Runtime の CPU 推論を使います。AMF でデコード
できない入力は FFmpeg ソフトウェアデコードへフォールバックします。セカンダリ復元と
区間スマートレンダリングは現在 NVIDIA 専用です。

`--device cuda:N` は PyTorch の GPU を選択し、Linux では MIGraphX の GPU も
選択します。FFmpeg 8 の Linux AMF デバイスコンテキストは現在アダプター引数を
無視するため、複数 GPU の AMD ホストでは AMF デコード/エンコードが既定の Vulkan
アダプターを使う場合があります。AMF アダプターを確実に選ぶ必要がある場合は、
コンテナまたはホスト側で対象 GPU のみを公開してください。
